      ...
      <div>
        ...
        >
          <v-flex class="mt-10" xs10 sm8 md6 lg5 justify-center>
            <v-card class="mt-5 ma-3" width="auto" elevation="2">
        ...
        <v-layout class="mt-10" justify-center="" align-start="">
          <v-flex xs10 sm8 md7 lg6>
            <div v-if="login">
              <h3 class="ml-7 h4">{{ views }} members viewed this page</h3>
              <div class="display-1 mb-5">Messages from viewers</div>
              <v-card
                elevation=""
                v-for="mess in messages"
                :key="mess.timestamp"
                class="card text-start my-2"
              >
...
export default {
  ...
  methods: {
    Signin() {
      console.log("jhgf");
      if (
        this.email == "prashanth31399@gmail.com" &&
        this.password == "prashanth205"
      ) {
        this.login = true;
      } else {
        this.erro = "true";
    ...
    date(time) {
      ...
      if (parseInt(time.getHours()) > 12) {
        ...
        if (time.getMinutes() >= 10) messtime = messtime + time.getMinutes();
        else messtime = messtime + "0" + time.getMinutes();
